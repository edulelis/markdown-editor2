...
class Markdown_Editor {
	...
	private function __construct() {
		...
		add_filter( 'media_send_to_editor', array($this, 'media_send_markdown_to_editor'));
	...
	/**
	 * Send media as markdown to editor
	 *
	 * @since 0.1.1
	 * @return void
	 */
	function media_send_markdown_to_editor($html) {
		  $id = $_POST['attachment']['id'];
			$output = (wp_attachment_is_image( $id ) ? '!' : '') . "[" . get_the_title($id) . "](" . wp_get_attachment_url($id) . ")\n";
			return $output;
	}
...
<?php echo file_get_contents(plugin_dir_path( __FILE__ ) ."../assets/scripts/markdown.js"); ?>
</script>
		<?php
	}
